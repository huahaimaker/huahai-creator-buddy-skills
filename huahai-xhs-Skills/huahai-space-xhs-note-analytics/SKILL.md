...
- 标题改写 → `huahai-space-xhs-title`；封面只做数据归因与改进建议，不负责出图
- 抓取平台数据、刷量刷互动、批量起号
...
| 表现 | 归因 | 该改什么 |
|---|---|---|
| CTR 低 + 封面文字多/小 | 信息流里缩略图看不清 | 封面主文案压到 6–10 字，字号加大 |
...
如果动作指向标题，接力 `huahai-space-xhs-title`；如果指向封面，只给可验证的文案、字号和信息层级建议。
