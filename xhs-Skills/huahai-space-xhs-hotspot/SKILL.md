...
# 小红书热点搜集（huahai-space-xhs-hotspot）

给创作者用的选题雷达。目标不是把数据倒出来，而是**回答三个问题**：这个赛道现在有没有流量？流量集中在哪几种内容形态上？我下一篇该写什么。

**产出链路**：本 skill 出选题 → `huahai-space-xhs-title` 起标题 → `huahai-space-xhs-writer` 写正文。所以最后一段必须是**可以直接粘给下游的选题卡**，不是一句"建议多观察"。
...
### 2. 标题钩子分布
统计 top 20 标题里出现的钩子：数字（"5 个""3 天"）、人群标签（"打工人""学生党"）、痛点原话（带引号的吐槽）、反差（"没想到""原来"）、身份背书（"三甲医生""HR"）、时效（"2026""最新"）。哪种钩子重复出现最多，就是这个赛道读者当下吃的那一套——直接传给 `huahai-space-xhs-title`。
...
```bash
S=~/.claude/skills/huahai-space-xhs-hotspot/scripts
...
| 词层"独有词" | 该赛道的专属搜索入口，交给 `huahai-space-xhs-title` |
...
每条按这个卡片给，让用户能整条粘给 `huahai-space-xhs-title` / `huahai-space-xhs-writer`：
...
└── references/
    ...
    ├── xhs_hot_article_format.md      # 红狐接口字段说明（同样来自 huahai-xhs-hotnotes）
