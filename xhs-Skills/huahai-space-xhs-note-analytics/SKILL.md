...
**不做**：
- 账号整体定位/主页/人设诊断 → 交给 `huahai-space-xhs-account-audit`
- 封面改稿 → `huahai-xhs-html`；标题改写 → `huahai-space-xhs-title`
- 抓取平台数据、刷量刷互动、批量起号
...
| 曝光低 + 历史笔记也普遍低 | 账号标签乱，系统不知道推给谁 | **这是账号层问题 → 转 `huahai-space-xhs-account-audit`** |
...
| 表现 | 归因 | 该改什么 |
|---|---|---|
| CTR 低 + 封面文字多/小 | 信息流里缩略图看不清 | 封面主文案压到 6–10 字，字号加大 → `huahai-xhs-html` |
| CTR 低 + 封面漂亮但没信息 | 纯美图无钩子，用户不知道点进去有什么 | 封面加一句利益点/悬念 |
| CTR 低 + 标题平淡 | 没制造"跟我有关"的判断 | 标题改写 → `huahai-space-xhs-title` |
...
| 涨粉率长期低、跨笔记一致 | 主页承接不行（简介没说清"关注我能持续得到什么"） | **账号层问题 → `huahai-space-xhs-account-audit`** |
...
如果动作指向封面/标题，直接接力：「具体怎么改封面，可以用 `huahai-xhs-html` 出几版方案」。
