...
- **文字/脚本**：直接抽 beats。
- **视频/逐字稿**：先读内容（`huahai-space-video-topic` 的逐字稿），抽出讲了哪几件事 → beats。
- 缺参数不追问，除非答案会改变产出路线。默认竖屏 720×896 或按 `huahai-space-video-edit` 配置，默认 30–40 秒。
...
- 上游：`huahai-space-video-script` 的 beats/画面需求，或用户一段文字/一条视频
- 下游：`04_broll/<名字>.mp4` → `huahai-space-video-edit` 合成，或直接单发
