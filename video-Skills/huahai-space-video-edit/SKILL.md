...
# huahai-space-video-edit · 口播剪辑与成片
...
每次先读配置，决定成片比例和动画风格。B-roll/动画画面本身交 `huahai-space-video-broll` 生成，本阶段只负责**对齐和合成**。

## 交接

- 上游：`huahai-space-video-script` 的 `script.md`（分镜依据）、录制的原视频
- 平行：B-roll 找 `huahai-space-video-broll`，字幕精修找 `huahai-space-video-subtitle`
- 下游：成片 → `huahai-space-video-cover` 出封面
