...
# huahai-space-video-audio · 配音与配乐
...
- 生成后走 `huahai-space-video-subtitle` 用**旁白音频**重转写做字幕对齐（TTS 段时长只是规划元数据，不能当字幕时间轴）
...
- 上游：`huahai-space-video-script` 的口播稿（→旁白）、`huahai-space-video-edit` 的成片、`huahai-space-video-broll` 的 B-roll
- 下游：带声音的成片 → 发布，或回 `huahai-space-video-edit` 做最终封装
